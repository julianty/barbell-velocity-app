...
## 5. New phone-width layout branch has zero test coverage — DONE
Added `phone-width layout renders results without overflow` to `test/widget_test.dart`: sets `tester.view.physicalSize = Size(390, 844) * 3` / `devicePixelRatio = 3` before running the happy-path pipeline, then scrolls the single-column `ListView` (via `scrollUntilVisible`) to confirm the charts, rep cards, and player all render with no exceptions, and that the wide branch's `SizedBox(width: 360)` side-by-side video column is absent. 123/123 tests pass, `flutter analyze` clean.
